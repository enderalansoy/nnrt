...
/**
 * \brief Callback function to be notified about models found by pareto
 *        optimization search
 *
 * This callback function can be used to be notified about the models found by
 ...
 * current search is aborted
 */
typedef int (*msat_pareto_model_callback)(msat_model_iterator it,
                                          void *user_data);
 ...
 * in case of errors.
 */
msat_objective msat_push_minimize(msat_env e, msat_term term, const char *lower,
                                  const char *upper, bool bvsigned = false);
 ...
 * in case of errors.
 */
msat_objective msat_push_maximize(msat_env e, msat_term term, const char *lower,
                                  const char *upper, bool bvsigned = false);
 ...
 *
 * \param env The environment in which to operate.
 * \param func The user-defined pareto call-back function. If it is NULL,
 *             attempting to perform a pareto-optimization search will
 *             yield an error.
 * \param user_data Generic data pointer which will be passed to \a func. Can
 *                  be anything, its value will not be interpreted.
 * \return zero on success, nonzero on error
...
int msat_set_pareto_model_callback(msat_env env,
                                   msat_pareto_model_callback func,
                                   void *user_data);
